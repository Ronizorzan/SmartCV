...
if uploaded_file:
    ...
    # Layout da página
    st.markdown("<h1 style='text-align: center'>💡 Triagem Inteligente (CV + GitHub)</h1>", unsafe_allow_html=True)
    ...
    input_col.markdown("<h3 style='text-align: center'>⚙️ Configurações da Análise</h3>", unsafe_allow_html=True)
    role = input_col.selectbox("Cargo alvo da análise:", ["Cientista de Dados", "Analista de Dados", "Engenheiro de Dados", 
        "Engenheiro de IA", "Engenheiro de Machine Learning", "Desenvolvedor Full Stack"])
    level = input_col.selectbox("Nível de senioridade", ["Júnior", "Pleno", "Sênior"])
            ...
            with st.spinner("🔎 Buscando repositórios e gerando parecer final"):                
                ...
                if extracted_data:                
                    with tabs[0]:
                        ...
                        # Exibição do resumo e usuário do GitHub
                        st.markdown("\n --------- \n") 
                    ...
                    with tabs[1]:
                        ...
                        if github_user:
                            ...
                            if github_docs:
                                st.markdown(f"<h2 style='text-align: center'>Repositórios analisados - Usuário: {github_user}</h2>", unsafe_allow_html=True)
                                st.toast(f" Analisando {len(github_docs)} repositórios de {github_user}", icon="⌛", duration="short")
                                st.markdown("------")
                    ...
                    with tabs[2]:                                                
                        final_analysis = analyze_with_github(extracted_data, github_docs, role, level)
                        st.markdown(final_analysis.replace("<br>", "\n"))
                        st.toast("Relatório gerado com sucesso", duration="long", icon="✅")
